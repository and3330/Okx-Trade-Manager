  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      <section className="px-4 py-3 border-b border-border">
        <div className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground mb-2">Model Leaderboard</div>
            ...
            <div key={row.providerId} className="flex items-center justify-between border border-border rounded-md p-2 bg-background/40">
              <div className="min-w-0">
                ...
                <div className="text-[10px] text-muted-foreground font-mono">
                  Suggest {row.totalSuggestions} · Exec {row.executedCount} · W/L {row.winCount}/{row.lossCount}
                </div>
              </div>
              <div className="text-right">
                ...
                <div className="text-[10px] text-muted-foreground">{(row.winRate * 100).toFixed(1)}% win</div>
              </div>
            </div>
          ))}
          {lbList.length === 0 && <div className="text-[11px] text-muted-foreground">No data yet — waiting for executions.</div>}
        </div>
      </section>

      {/* Executions */}
      <section className="px-4 py-3 border-b border-border">
        <div className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground mb-2">Recent executions ({execList.length})</div>
          ...
          {execList.slice(0, 25).map((e) => (
            <div key={e.id} className="border border-border rounded-md p-2 bg-background/40">
              ...
              <div className="flex items-center justify-between mt-1 text-[10px] text-muted-foreground font-mono">
                ...
                <span>{e.entryPrice != null ? `entry ${e.entryPrice}` : ""}</span>
              ...
              {e.realizedPnlUsdt != null && (
                <div className={cn("text-[11px] font-mono font-bold mt-1", e.realizedPnlUsdt >= 0 ? "text-[#00e59b]" : "text-[#ff4d4d]")}>
                  PnL: {e.realizedPnlUsdt >= 0 ? "+" : ""}{e.realizedPnlUsdt.toFixed(2)} USDT
                </div>
              )}
              {e.reason && e.status !== "submitted" && (
                <div className="text-[10px] text-destructive/80 mt-1 break-all">{e.reason}</div>
              )}
              {e.chosenProviderId && <div className="text-[9px] text-muted-foreground mt-1">via {e.chosenProviderId}</div>}
            </div>
          ))}
          {execList.length === 0 && <div className="text-[11px] text-muted-foreground">No executions yet.</div>}
        </div>
      </section>

      {/* Recent decisions */}
      <section className="px-4 py-3">
        <div className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground mb-2">Recent decisions ({decList.length})</div>
        <div className="space-y-1">
          {decList.slice(0, 15).map((d) => (
            <div key={d.id} className="border border-border rounded-md p-2 bg-background/40 text-[11px] font-mono">
              ...
              <div className="flex items-center gap-2 mt-1 text-[10px]">
                ...
                )}>{d.consensusAction ?? "hold"}</span>
                {d.consensusConfidence != null && <span className="text-muted-foreground">{d.consensusConfidence}/10</span>}
                <span className="text-muted-foreground">[{d.triggeredBy}]</span>
              </div>
            </div>
          ))}
          {decList.length === 0 && <div className="text-[11px] text-muted-foreground">No decisions yet.</div>}
        </div>
      </section>
    </div>
  );
