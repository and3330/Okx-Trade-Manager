  ...
  return (
    <div className="flex h-full flex-col">
      <div className="px-4 py-2 border-b border-border bg-card">
        <span className="text-xs font-semibold text-muted-foreground tracking-wider uppercase">市場成交</span>
      </div>
      <ScrollArea className="flex-1">
        {isLoading ? (
          <div className="p-4 text-center text-sm text-muted-foreground">載入成交中...</div>
        ) : fills?.length === 0 ? (
          <div className="p-4 text-center text-sm text-muted-foreground">尚無成交</div>
        ) : (
